...
void main() {
  ...
  test('deterministic ids: match formula and are unique', () {
    ...
    for (final rem in r) {
      expect(
        rem.id,
        PrayerReminder.deterministicId(rem.dayKey, rem.prayerName),
      );
    }
  ...
  test(
    'reschedule cancels own reminders first, then schedules (no dup)',
    () async {
      ...
      // TASK 096: yöntem artık zorunlu parametredir. Bu testin konusu
      // iptal/çift-kurulum davranışıdır, yöntem değil — mevcut varsayılan
      // verilir ve beklenen sayılar DEĞİŞMEZ.
      final first = await scheduler.reschedule(
        coordinates: istanbul,
        copy: copy,
        method: PrayerTimeCalculationMethod.defaultMethod,
      );
  ...
  test('inexact capability is reported through the outcome', () async {
    ...
    final outcome = await scheduler.reschedule(
      coordinates: istanbul,
      copy: copy,
      method: PrayerTimeCalculationMethod.defaultMethod,
    );
...
final class _FakeNotifications implements LocalNotificationService {
  ...
  Future<void> cancelAllPrayerReminders() async {
    cancelAllCalls++;
    scheduled
        .clear(); // gerçek servis payload'a göre yalnız kendi id'lerini siler
  }
